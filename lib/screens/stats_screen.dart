  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: solves.isEmpty
          ? Center(
              child:
                  Text('Nessun dato ancora', style: theme.textTheme.bodyMedium))
          : ListView(
              ...
              children: [
                ...
                GridView.count(
                  ...
                  children: [
                    ...
                    StatsCard(
                        label: 'MIGLIORE',
                        valueMs: session.bestTime,
                        highlight: true,
                        accentColor: accent),
                    StatsCard(
                        label: 'MEDIA SESSIONE',
                        valueMs: session.sessionMean,
                        accentColor: accent),
                    StatsCard(
                        label: 'AO5',
                        valueMs: session.ao5,
                    ...
                    StatsCard(
                        label: 'AO12',
                        valueMs: session.ao12,
                        accentColor: accent),
                    StatsCard(
                        label: 'AO50',
                        valueMs: session.activeSession?.averageOf(50),
                        accentColor: accent),
                  ],
                ),
...
class _Label extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Text(text,
      style:
          theme.textTheme.labelSmall?.copyWith(letterSpacing: 2, fontSize: 11));
}
...
class _TimeChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      onTapDown: onHover == null
          ? null
          : (d) {
              final idx =
                  ((d.localPosition.dx / (d.globalPosition.dx / data.length))
...
class _ChartPainter extends CustomPainter {
  ...
  _ChartPainter(
      {required this.times, required this.accent, required this.gridColor});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final pts = <Offset>[
      for (int i = 0; i < times.length; i++)
        Offset(
            (i / (times.length - 1)) * w, h - ((times[i] - minT) / range) * h)
    ];
...
class _DeltaPanel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    for (int i = 1; i < valid.length; i++) {
      deltas.add(
          valid[i].effectiveMilliseconds - valid[i - 1].effectiveMilliseconds);
    }
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(children: [
          _DeltaStat(
              'Media Δ',
              _fmtDelta(avgDelta.round()),
              avgDelta < 0 ? const Color(0xFF30D158) : const Color(0xFFFF453A),
              theme),
          const SizedBox(width: 10),
          _DeltaStat(
              'Migl. Δ', _fmtDelta(minDelta), const Color(0xFF30D158), theme),
          const SizedBox(width: 10),
          _DeltaStat(
              'Pegg. Δ', _fmtDelta(maxDelta), const Color(0xFFFF453A), theme),
        ]),
        const SizedBox(height: 10),
        // Barra miglioramenti
        Row(children: [
...
class _DistributionBar extends StatelessWidget {
  ...
  const _DistributionBar(
      {required this.solves, required this.accent, required this.theme});
  @override
  Widget build(BuildContext context) {
    final v = solves
        .where((s) => s.isValid)
        .map((s) => s.effectiveMilliseconds)
        .toList()
      ..sort();
    ...
    for (final t in v) {
      counts[((t - min) / (range / n)).floor().clamp(0, n - 1)]++;
    }
    ...
    return Container(
      ...
      child: Column(children: [
        ...
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          Text(SolveTime.format(min),
              style: theme.textTheme.bodyMedium?.copyWith(fontSize: 10)),
          Text(SolveTime.format(max),
              style: theme.textTheme.bodyMedium?.copyWith(fontSize: 10)),
        ]),
      ]),
    );
