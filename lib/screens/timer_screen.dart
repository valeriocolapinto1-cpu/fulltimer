...
class _TimerScreenState extends State<TimerScreen> {
  ...
  void _setup() {
    ...
    _timerProvider = timer;
    final s = context.read<SettingsProvider>();
    timer.configure(
        inspectionEnabled: s.inspectionEnabled,
        holdDurationMs: s.holdDuration,
        inspectionDuration: s.inspectionDuration,
    ...
    timer.onTimerStop = (ms) {
      ...
      if (ms > 0) {
        ...
        se.updateSolveResult(sv.id, SolveResult.dnf);
      }
    };
  }
  ...
  @override
  void dispose() {
    _timerProvider?.onTimerStop = null;
    super.dispose();
  }
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      ...
      content: Row(children: [
        ...
        GlassButton(
            ...
            onTap: () {
              final sv = se.currentSolves;
              if (sv.isNotEmpty) {
                se.updateSolveResult(sv.last.id, SolveResult.plusTwo);
              }
        ...
        GlassButton(
            ...
            onTap: () {
              final sv = se.currentSolves;
              if (sv.isNotEmpty) {
                se.updateSolveResult(sv.last.id, SolveResult.dnf);
              }
              ScaffoldMessenger.of(context).hideCurrentSnackBar();
            },
            child: Text('DNF',
                style: GoogleFonts.nunito(
                    fontWeight: FontWeight.w700, color: th.colorScheme.error))),
      ]),
    ));
  }
  ...
  Widget build(BuildContext context) {
    ...
    final isActive = timer.state == TimerState.running ||
        timer.state == TimerState.holding ||
        timer.state == TimerState.ready ||
        timer.state == TimerState.inspection ||
        timer.state == TimerState.holdingFromInspection ||
        timer.state == TimerState.readyFromInspection;
        ...
        AnimatedOpacity(
            ...
            child: IgnorePointer(
                ...
                child: Column(children: [
                  ...
                  EventSelector(
                      events: se.allEvents,
                      activeEventId: se.activeEventId,
                      onEventSelected: (id) {
                        se.switchEvent(id);
                  ...
                  _ScrambleBar(
                      scramble: se.currentScramble,
                      onRefresh: se.newScramble,
                      accentColor: accent,
                      eventId: se.activeEventId,
                      showPreview: s.showScramblePreview),
                ]))),
        const Spacer(),
        Listener(
          onPointerDown: (_) {
            context.read<TimerProvider>().configure(
                inspectionEnabled: s.inspectionEnabled,
                holdDurationMs: s.holdDuration,
                inspectionDuration: s.inspectionDuration,
                soundEnabled: s.soundEnabled,
          ...
          child: SizedBox(
              ...
              child: Center(
                ...
                inspectionSecondsLeft: timer.inspectionSecondsLeft,
                isInspectionWarning: timer.isInspectionWarning,
                accentColor: accent,
                displayMode: s.timerDisplay,
              ))),
        ),
        const Spacer(),
        AnimatedOpacity(
...
class _ManualInputState extends State<_ManualInput> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SafeArea(
        ...
        const SizedBox(height: 8),
        EventSelector(
            events: widget.session.allEvents,
            activeEventId: widget.session.activeEventId,
            onEventSelected: widget.session.switchEvent,
            accentColor: widget.accent),
        const SizedBox(height: 4),
        _ScrambleBar(
          scramble: widget.session.currentScramble,
        ...
        Container(
          ...
          child: Column(children: [
            Text('hh:mm:ss.ms',
                style: th.textTheme.labelSmall?.copyWith(letterSpacing: 2)),
            const SizedBox(height: 8),
  ...
  Widget _numBtn(String d, ThemeData th, Color onSurf) => GlassButton(
      ...
      child: Text(d,
          style: GoogleFonts.nunito(
              fontSize: 24, fontWeight: FontWeight.w600, color: onSurf)));
}
        ...
        child: Column(children: [
          Row(children: [
            ...
            GestureDetector(
                onTap: onRefresh,
                child:
                    Icon(Icons.refresh_rounded, color: accentColor, size: 20)),
          ]),
          if (showPreview) ...[
            const SizedBox(height: 8),
            Center(
                child: ScramblePreview(
                    scramble: scramble, eventId: eventId, size: 120)),
          ],
        ]));
  }
}
...
class _SessionSel extends StatelessWidget {
  ...
  const _SessionSel(
      {required this.session, required this.accent, required this.theme});
    ...
    showModalBottomSheet(
        ...
        builder: (c) => Padding(
            ...
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              Text('Sessioni — ${session.activeEvent.name}',
              ...
              ...session.sessionsForActiveEvent.map((s) {
                ...
                return ListTile(
                  ...
                  subtitle: Text('${s.solves.length} solve',
                      style:
  ...
  void _dlg(BuildContext ctx, String title, String hint, String initial,
  ...
  void _newDlg(BuildContext ctx) => _dlg(ctx, 'Nuova sessione', 'Nome...', '',
      (name) => session.newSession(name: name.isEmpty ? null : name));
  void _renameDlg(BuildContext ctx, String id, String cur) => _dlg(
      ctx, 'Rinomina', cur, cur, (name) => session.renameSession(id, name));
}
